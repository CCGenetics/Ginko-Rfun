...
Which country(ies) is(are) being analysed?
```{r, echo=F, message=F, warning=F}
...
`r desired_country` has a total of `r nrow(indicators_full)` assessments (data entries in the Kobo form) where it was attempted to estimate the Ne 500 or PM indicators. These assessments where done in a total of `r length(unique(indicators_full$taxon))` taxa (species or subspecies). 

The number of assessments and the number of taxa can differ, because some taxa could have been assessed more than once, for example to account for different methods to define populations.
...
# mean Ne 500 indicator for the country
mean_PM <- mean(indicators_full$indicatorPM, na.rm = TRUE)

# median Ne 500 indicator for the country
median_PM <-median(indicators_full$indicatorPM, na.rm = TRUE)
```

There was enough data to estimate the PM indicator in `r sum(!is.na(indicators_full$indicatorPM))` taxa out of `r length(unique(indicators_full$taxon))` assessed. 

Considering only the taxa where it was possible to estimate the Ne 500 indicator, the indicator mean value for the country is `r mean_PM`. That means that in the analysed species `r round(mean_PM*100, 1)`% of their populations are still existing.

#### Figure 3. Species-level proportion of populations above Ne 500
```{r, echo=F, message=F, warning=F}
indicators_full %>%
  filter(!is.na(indicatorPM)) %>%
  
  ggplot(aes(x = indicatorPM, y = reorder(taxon, indicatorPM))) + # order by indicator value
  geom_col(fill = "#004D40") +
  geom_vline(xintercept = mean_PM, # this is estimated in code outside of the plot, above
             colour = "#FFC107",
             linetype = "dashed",
             linewidth = 0.8) +
  geom_text(aes(label = round(indicatorPM, 2),
                x = indicatorPM + 0.02),                 # nudge labels slightly right
            size = 3,
            hjust = 0) +
  
  scale_x_continuous(expand = expansion(mult = c(0, 0.02)),
                     limits = c(0, 1)) +                 # adjust if indicatorPM is 0–1
  
  labs(
    x = "Proportion of populations maintained within species",
    y = "",
    title = ""
  ) +
  theme_minimal(base_size = 12) +
...
#### Figure 4. The frequency distribution of the Ne 500 indicator

This figure shows how the indicator value is distributed across all assessed species. A bias towards the left (indicator value of 1) mean that in the majority of species all populations are still maintained (not extinct).
...
indicators_full %>%
  filter(!is.na(indicatorPM)) %>%
  ggplot(aes(x = indicatorPM)) +
  geom_histogram(fill="#004D40") + # Adjust the number of bins as needed
  labs(x = "Proportion of populations maintained within species", y = "Frequency") +
